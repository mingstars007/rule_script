...
if (responseData?.MSPS_ENTITY?.Coupon_NAME) {
  ...
  message += `活动开始时间：${parseAndFormatTime(responseData.MSPS_ENTITY.EFFECT_PERIOD_START)}\n`;
  message += `活动结束时间：${parseAndFormatTime(responseData.MSPS_ENTITY.EFFECT_PERIOD_END)}\n`;
  if (isTimeGreaterThanNow(parseAndFormatTime(responseData.MSPS_ENTITY.EFFECT_PERIOD_START))) {
    message += `当前时间早于活动时间，已经将活动设为开始时间后1分钟！\n`;
    responseData.SYSTEM_TIME = getTimeOffset(parseAndFormatTime(responseData.MSPS_ENTITY.EFFECT_PERIOD_START), 1, "minutes");
    isChange = true;
  } else {
    message += `活动已经开始！\n`;
  }
  ...
  message += `活动总名额：${responseData.KHHK_ENTITY.DcCp_Issu_Num}\n`;
  message += `活动剩余名额：${responseData.KHHK_ENTITY.Rvl_Rcrd_Num}\n`;
  if (responseData.KHHK_ENTITY.Rvl_Rcrd_Num <= 0) {
    message += `活动已没有名额！改为有名额以点亮按钮！\n`;
    responseData.KHHK_ENTITY.Rvl_Rcrd_Num = 1;
    isChange = true;
  }
  if (isChange) {
    $.msg($.name, message);
...
function getTimeOffset(date, amount, unit, direction = "after") {
  // 创建一个新的 Date 对象，防止修改原始对象
  ...
  // 计算偏移的毫秒数，根据 direction 判断是加还是减
  const offsetInMs = amount * unitConversion[unit];
  const timeInMs = direction === "before" ? newDate.getTime() - offsetInMs : newDate.getTime() + offsetInMs;

  // 设置新的时间戳
  newDate.setTime(timeInMs);
